...
|---|------------------------------------------------------------------|---|---|
| 1 | Сервис запускается по инструкциям из `project/README.md`         | ✅ | `README.md`, `src/service/app.py`, endpoint `/health` |
| 2 | Endpoint `/predict` использует реальную обученную модель         | ✅ | `src/models/predict.py`, `artifacts/best_model.joblib` |
...
| 5 | Код структурирован в `src/`                                      | ✅ | `src/data/`, `src/models/`, `src/service/` |
| 6 | Есть Dockerfile и/или понятный сценарий развёртывания без Docker | ✅ | `Dockerfile`, `.dockerignore`, контейнерный `pytest` |
...
| 10 | `README.md` и `report.md` описывают запуск и использование       | ✅ | локальный запуск, FastAPI, Streamlit UI, Docker |
